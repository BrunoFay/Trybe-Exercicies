...
export class PostsController {
  ...
  public getPostsByQuery: RequestHandler = async (req, res, next) => {
  ...
  public getPostById: RequestHandler = async (req, res, next) => {
    const postId: number = Number(req.params.id)
    const post: IPostDB = await this.service.getPostById(postId)
    return res.status(200).json(post)
  }
  public creatPost: RequestHandler = async (req, res, next) => {
  ...
  public updatePost: RequestHandler = async (req, res, next) => {
    ...
    const editedPost = await this.service.updatePost(req.body, postId)
    return res.status(202).json(editedPost)
  }
  public removePost: RequestHandler = async (req, res, next) => {
    const postId: number = Number(req.params.id)
    const ckeckIfPostExistInDb = await this.service.getPostById(postId)
    if (!ckeckIfPostExistInDb) return res.status(404).json({ message: 'post notfound' })
